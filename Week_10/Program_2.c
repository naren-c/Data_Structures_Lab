...
void insert_to_hash(struct element*, int, int, char*, int*,long long int*);
...
int main()
{
 ...
 scanf("%lld",&phone);
 insert_to_hash(hashtable,tablesz,key,name,&no_elements,&phone);
 break;
...
printf("Key = %d name = %s mobile= 
%d\n",ht[index].key,ht[index].name,ht[index].phone);
return;
}
...
i++;
 ...
 printf("%d :", i);
 if (ht[i].mark!=0)
 printf("Key = %d, name = %s, mobile=%lld\n",ht[i].key,ht[i].name,ht[i].phone);
 printf("\n");
 }
 } 
 
void insert_to_hash(struct element *ht, int size, int key, char *name, int *count,long long int* 
phone)
 {
 int index;
 if(size==*count)
